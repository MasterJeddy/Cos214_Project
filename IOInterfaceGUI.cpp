...
bool IOInterfaceGUI::OnUserUpdate(float fElapsedTime) {
  ...
  if (toggleDebug) {
    ...
    for (int i = 0; i < orderQueueSize; i++) {
      ...
      DrawStringDecal({30, static_cast<float>(25 + 10 + 10 * i)},
                      std::to_string(order->orderNumber) + " " + order->tableID);
    ...
    for (int i = 0; i < orderQueueSize; i++) {
      ...
      DrawStringDecal({30, static_cast<float>(25 + 10 + 10 * FinishedOrdersOffset + 10 * i)},
                      std::to_string(order->orderNumber) + " " + order->tableID);
    ...
    for (it->first(); !it->isDone(); it->next()) {
      ...
      case COMMANDS::SAVE:DrawStringDecal({720, static_cast<float>(5 + 10 * offset)}, "Save");
        break;
      case COMMANDS::LOAD:DrawStringDecal({720, static_cast<float>(5 + 10 * offset)}, "Load");
        break;
      case COMMANDS::TOGGLE_HELP:DrawStringDecal({720, static_cast<float>(5 + 10 * offset)}, "Toggle Help");
        break;
      case COMMANDS::TOGGLE_LOG:DrawStringDecal({720, static_cast<float>(5 + 10 * offset)}, "Toggle Log");
        break;
      case COMMANDS::HIRE_MAITRE_D:DrawStringDecal({720, static_cast<float>(5 + 10 * offset)}, "Hire Maitre D");
        break;
      case COMMANDS::BUY_TABLE:DrawStringDecal({720, static_cast<float>(5 + 10 * offset)}, "Buy Table");
        break;
      case COMMANDS::EXPAND_FLOOR:DrawStringDecal({720, static_cast<float>(5 + 10 * offset)}, "Expand Floor");
        break;
      case COMMANDS::HIRE_WAITER:DrawStringDecal({720, static_cast<float>(5 + 10 * offset)}, "Hire Waiter");
        break;
      case COMMANDS::UPDATE:DrawStringDecal({720, static_cast<float>(5 + 10 * offset)}, "Update");
        break;
      case COMMANDS::EXPAND_KITCHEN:DrawStringDecal({720, static_cast<float>(5 + 10 * offset)}, "Expand Kitchen");
        break;
      case COMMANDS::HIRE_CHEF:DrawStringDecal({720, static_cast<float>(5 + 10 * offset)}, "Hire Chef");
        break;
      case COMMANDS::BUY_STOCK:DrawStringDecal({720, static_cast<float>(5 + 10 * offset)}, "Buy Stock");
        break;
      }
    }
    ...
    //Draw Controls
    DrawStringDecal({5, 705},
                    "U-Update H-Hire Chef W-Hire Waiter M-Hire Waiter B-Buy Table S-Save L-Load Q-Quit D-Dump Save to file F-Load Save from File");
